...
- **로더 버전은 자유 변수**: 업데이트할 모드들의 [min,max] 교집합을 만족하도록 위/아래 어느 방향으로든 이동(범위 내 **최신 안정 릴리스** = 내릴 땐 최소한의 다운그레이드). 현재가 범위 내이거나 제약이 없으면 변경하지 않는다. 고정은 game_version·loader_type뿐. (2026-07-10 사용자 확정 — 초기 "다운그레이드 금지"는 오독)
...
- 2026-07-10 정책 정정: is_upgrade 가드·retain_loader_compatible 제거(다운그레이드 허용, 교집합이 정합 보장).
