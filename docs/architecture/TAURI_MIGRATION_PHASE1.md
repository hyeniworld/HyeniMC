...
- **M6 — 마감** — **코드 부분 완료 (2026-07-06, `d0bff9f`/`6cbd123`)**: M6a(env 빌드 주입 build.rs — AZURE_CLIENT_ID/HYENIMC_WORKER_URL 바이너리 임베드 검증 / 런처 자동 업데이트 커맨드 4종 tauri-plugin-updater / 경고 정리) + M6b(제작자 UI 숨김 3곳 isCreatorMode / NSIS 교체 훅 installer-hooks.nsh / [QA_AND_RELEASE.md](QA_AND_RELEASE.md)). **인프라/실기 잔여**: updater endpoint 실서버 구성(현 placeholder), 코드 서명(Win Authenticode/mac Developer ID), Squirrel.Mac 교체 검증, 브릿지 릴리스, QA 매트릭스 전체 회귀 — 전부 일괄 테스트/배포 결정 단계
