...
- **M2 — 게임 파이프라인**: 다운로드 엔진(병렬/SHA1/재개/진행 이벤트), 버전 매니페스트/에셋/라이브러리, Java 감지, JVM 실행 + 로그 스트림 + 종료/크래시 감지. **바닐라 실행 성공이 완료 조건**
  ...
  - **M2b (다음)**: 설치 파이프라인(버전 jar/라이브러리/natives/에셋 오케스트레이션 — 공유 디렉터리 `<userData>/shared/libraries|assets` + 프로필별 `<instance>/versions/<id>` 실측 레이아웃), natives 추출(zip), JVM 인자 조립(${변수} 치환)+spawn+로그 스트림, Tauri 커맨드/이벤트 배선(shim `on` 이벤트 브리지 포함)
