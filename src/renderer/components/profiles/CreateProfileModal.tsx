...
import type { CreateProfileData } from '../../../shared/types';
...
export function CreateProfileModal({ onClose, onSuccess }: CreateProfileModalProps) {
  ...
  React.useEffect(() => {
    const loadVersions = async () => {
      try {
        ...
        setFormData(prev => ({ ...prev, gameVersion: latest }));
      } catch (err) {
        ...
        setFormData(prev => ({ ...prev, gameVersion: fallback[0] }));
      } finally {
  ...
  const handleChange = (field: keyof CreateProfileData, value: string) => {
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
      <div className="card max-w-lg w-full shadow-2xl animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          {/* Loader Version - 향후 API 연동 예정 */}
          {formData.loaderType !== 'vanilla' && (
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-3">
              <p className="text-sm text-gray-400">
                <span className="font-semibold text-gray-300">로더 버전:</span> 최신 버전이 자동으로 설정됩니다
              </p>
              <p className="text-xs text-gray-500 mt-1">
                (향후 업데이트에서 버전 선택 기능이 추가됩니다)
              </p>
            </div>
          )}
