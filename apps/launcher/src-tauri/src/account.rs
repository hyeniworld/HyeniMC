...
pub fn account_list(db: State<DbState>) -> Result<Vec<hyenimc_core::account::Account>, String> {
    hyenimc_core::account::list_accounts(&db.0.lock().unwrap()).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn account_remove(db: State<DbState>, id: String) -> Result<bool, String> {
    hyenimc_core::account::remove_account(&db.0.lock().unwrap(), &id).map_err(|e| e.to_string())
}
